...
.crumb{{font-size:11px;background:#f5f5f5;color:#555;border-radius:20px;padding:4px 10px;white-space:nowrap;border:none;cursor:pointer;font-family:inherit}}
.crumb:hover{{background:#e8e8e8;color:#222}}
...
.bisherige-body{{max-height:280px;overflow-y:auto;padding:10px 14px}}
.party-row{{margin-bottom:12px}}
.party-row:last-child{{margin-bottom:0}}
.party-badge{{font-size:10px;font-weight:700;letter-spacing:.5px;padding:2px 7px;border-radius:3px;color:white;display:inline-block;margin-bottom:5px}}
.party-names{{display:flex;flex-direction:column;gap:1px}}
.cand-link{{color:#444;text-decoration:none;padding:5px 0;border-bottom:1px solid #f0f0f0;font-size:14px;display:block;line-height:1.4}}
.cand-link:last-child{{border-bottom:none}}
.cand-link:hover{{color:#C8102E}}
...
.back-link{{background:none;border:none;color:#aaa;font-size:13px;cursor:pointer;padding:10px 0;margin-top:6px;display:flex;align-items:center;gap:4px;min-height:44px}}
...
const NODES = {{
  ...
  q5:{{type:'radio',step:4,q:'Welche Veränderung ist Ihnen besonders wichtig?',
    opts:[
      ...
      {{l:'Im Landrat soll es mehr Frauen haben.',next:'r_list_frauen',crumb:'Mehr Frauen'}},
      {{l:'Im Landrat soll es mehr Junge haben.',next:'r_list_junge',crumb:'Mehr Junge'}},
      {{l:'Im Landrat soll es mehr Landwirte, Gewerbebetreibende, Rechtsanwälte, Lehrer usw. haben.',next:'r_list_vielfalt',crumb:'Mehr Vielfalt'}},
  ...
  r_novote:{{type:'cont',icon:'🗳️',title:'Gehen Sie trotzdem!',
    ...
    next:'q3',nextLabel:'Weiter zu Frage 3 →'}},
  r_unch:{{type:'res',icon:'✅',title:'Liste unverändert einlegen',
    text:'Legen Sie die Liste Ihrer Wahl unverändert in die Urne.'}},
  r_list_frauen:{{type:'res',icon:'📋',title:'Die passende Liste wählen',
    text:'Nehmen Sie diejenige Liste mit den meisten Frauen unter den kumulierten (doppelt aufgeführten) Kandidaten und Kandidatinnen. Nichtkumulierte haben in der Regel weniger Wahlchancen.',
    link:{{href:'https://www.smartvote.ch/de/elections/26_gl_leg/lists',label:'→ Alle Listen auf smartvote.ch'}}}},
  r_list_junge:{{type:'res',icon:'📋',title:'Die passende Liste wählen',
    text:'Nehmen Sie diejenige Liste mit den meisten Jungen unter den kumulierten (doppelt aufgeführten) Kandidaten und Kandidatinnen. Nichtkumulierte haben in der Regel weniger Wahlchancen.',
    link:{{href:'https://www.smartvote.ch/de/elections/26_gl_leg/lists',label:'→ Alle Listen auf smartvote.ch'}}}},
  r_list_vielfalt:{{type:'res',icon:'📋',title:'Die passende Liste wählen',
    text:'Nehmen Sie diejenige Liste mit den meisten Landwirten, Gewerbebetreibenden, Rechtsanwälten, Lehrern etc. unter den kumulierten (doppelt aufgeführten) Kandidaten und Kandidatinnen. Nichtkumulierte haben in der Regel weniger Wahlchancen.',
    link:{{href:'https://www.smartvote.ch/de/elections/26_gl_leg/lists',label:'→ Alle Listen auf smartvote.ch'}}}},
  r_nordglarus:{{type:'res',icon:'📍',title:'Empfehlung für Glarus Nord / Glarus',
    ...
    text:'Nehmen Sie die leere Liste und wählen Sie alle Bisherigen Ihres Wahlkreises. Wer das ist, sagen Ihnen die Parteien in ihrer Werbung. Sie finden die Bisherigen auch auf der Website des Kantons: <a href=\\"https://www.gl.ch/parlament/landrat/mitglieder-des-landrates-20222026.html/234\\" target=\\"_blank\\" rel=\\"noopener\\" style=\\"color:#C8102E\\">gl.ch/parlament/landrat/mitglieder-des-landrates</a>'}},
  r_sued:{{type:'res',icon:'📍',title:'Empfehlung für Glarus Süd',
    district:'Glarus Süd',
    smartvote:'https://www.smartvote.ch/de/elections/26_gl_leg/candidacies',
    text:'Nehmen Sie die leere Liste und wählen Sie alle Bisherigen Ihres Wahlkreises. Damit wird Ihre Liste noch nicht voll, denn Hans Rudolf Forrer und Rahel Nassim Isenegger (beide SP) treten nicht mehr zur Wahl an. Füllen Sie die beiden leeren Linien mit Namen Ihrer Wahl. Beachten Sie dabei, dass am Ende ein Name nicht mehr als zwei Mal auf der Liste stehen darf. Wer die Bisherigen sind, sagen Ihnen die Parteien in ihrer Werbung. Sie finden die Bisherigen auch auf der Website des Kantons: <a href=\\"https://www.gl.ch/parlament/landrat/mitglieder-des-landrates-20222026.html/234\\" target=\\"_blank\\" rel=\\"noopener\\" style=\\"color:#C8102E\\">gl.ch/parlament/landrat/mitglieder-des-landrates</a>'}},
...
function renderCrumbs() {{
  ...
  el.innerHTML = history.map((h,i) =>
    ...
    `<button class="crumb" onclick="goToStep(${{i}})">${{h.label}}</button>`
  ).join('');
}}
...
function goToStep(i) {{
  const nodeId = history[i].nodeId;
  history = history.slice(0, i);
  const body = document.getElementById('body');
  body.style.opacity='0';
  setTimeout(()=>{{ render(nodeId); body.style.opacity='1'; }}, 120);
}}
